...
def createCheckersBoard(grid, name, captionLoc=True):
    n = len(grid[0])
    canvasSize = (n, n)
    fig, ax = createCanvas(canvasSize[0], canvasSize[1], my_dpi)
    color = "black"

    colordic = {0: "paleturquoise", 1: "thistle", 2: 'aquamarine', 3: 'lightpink'}
    letterdic = {0: "W", 1: "S", 2: 'E', 3: 'N'}
    for row in range(n):
        for col in range(n):
            num = grid[row, col]
            color = colordic[num]
            letter = letterdic[num]
